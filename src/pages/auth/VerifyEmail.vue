<template lang="">
    <div class="row login-page">
      ...
      <div class="col-6 right_column" style="padding:200px 120px 0px 120px">  
...
const verifyemail = () => {
  form.value.validate().then((success) => {
    if (success) {
      loading.value = true;
      userStore.VerifyEmail({ ...model.value })
        .then((response) => {
          loading.value = false;
          notify('Password updated successfully', "positive", "check_circle");
          router.push('/');
          loading.value = false;
        })
        .catch(() => {
          notify(response.data.message, "negative", "error");
          loading.value = false;
        });
    }
  });
